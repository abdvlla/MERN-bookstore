...
const BookSingleCard = ({ book }) => {
  ...
  return (
    <div className="relative p-4 m-4 border-2 border-gray-300 rounded-lg shadow-lg hover:shadow-xl ">
      <h2 className="absolute bottom-2 right-2 px-3 py-1 bg-gray-600 text-white rounded-lg">
        {book.publishYear}
      </h2>
      <h4 className="my-2 text-gray-500">{book._id}</h4>
      <div className="flex items-center gap-x-2">
        <PiBookOpenTextLight className="text-gray-600 text-2xl" />
      ...
      <div className="absolute top-2 right-2">
        <HorizontalDropdown book={book} setShowModal={setShowModal} />
      </div>
      {showModal && (
